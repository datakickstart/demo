...
Output: JSON files in /Volumes/main/demo_dw_raw/raw_data/
"""
...
# ── Spark Session (serverless with managed deps) ───────────────────────────────
...
cust_count = N_CUSTOMERS
print(f"  ✓ customers/     {cust_count:>10,} rows")
...
ord_count = N_ORDERS
print(f"  ✓ orders/        {ord_count:>10,} rows")
...
print("\ntotal_amount = sum(line_items) spot check (5 orders):")
computed_totals = (
    line_items_json
...
orders_json.join(computed_totals, on="order_id") \
    .select(
        "order_id",
        F.round("total_amount", 2).alias("total_amount"),
        "computed_total",
        (F.round(F.col("total_amount"), 2) == F.col("computed_total")).alias("matches"),
    ) \
    .limit(5).show()
