...
-- Parameters: ${catalog}, ${gold_schema} — set via pipeline configuration.
CREATE OR REFRESH MATERIALIZED VIEW ${catalog}.${gold_schema}.membership_metrics
...
order_agg AS (
  ...
  LEFT JOIN ${catalog}.${gold_schema}.orders_current o ON c.customer_id = o.customer_id
...
line_item_agg AS (
  ...
  FROM current_customers c
  LEFT JOIN ${catalog}.${gold_schema}.orders_current    o  ON c.customer_id = o.customer_id
  LEFT JOIN ${catalog}.${gold_schema}.line_items_current li ON o.order_id   = li.order_id
